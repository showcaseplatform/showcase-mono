// todo: this should not be deployed to prod environment
...
TestController.route('/loginWithUid').post(async (req, res) => {
  try {
    const { uid } = req.body
...
TestController.route('/testnotifications').get((req, res) => {
...
TestController.route('/test1').get((req, res) => {
...
TestController.route('/test2').get((req, res) => {
  db.collection('badgesales')
    .get()
    .then((querySnapshot) => {
      return querySnapshot.forEach((doc) => {
...
TestController.route('/testq').get(async (req, res) => {
